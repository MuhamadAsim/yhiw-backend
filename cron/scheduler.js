...
/**
 * Clean up expired jobs (pending/accepted that passed expiresAt)
 */
const cleanupExpiredJobs = async () => {
  try {
    const now = new Date();
    
    const expiredJobs = await Notification.find({
      status: { $in: ['pending', 'accepted'] },
      expiresAt: { $lt: now, $exists: true }
    }).select('bookingId serviceName status expiresAt');
    
    if (expiredJobs.length === 0) {
      return;
    }
    
    console.log(`\n🧹 Cleaning up ${expiredJobs.length} expired job(s):`);
    
    expiredJobs.forEach(job => {
      console.log(`   🕒 ${job.bookingId} - ${job.serviceName} (expired at ${job.expiresAt?.toISOString()})`);
    });
...
export const startScheduler = () => {
  cron.schedule('* * * * *', async () => {
    try {
      ...
      // Then, activate due scheduled jobs
  ...
  console.log('🕐 Job scheduler started with manual cleanup');
};

/**
 * Manual cleanup function (can be called from API for testing)
 */
